  -- YSI E-commerce: Initial Schema + RLS + Auth Sync
  -- Run this once in the Supabase SQL Editor.

  -- 0. Schema-level alterations (must run before CREATE TABLE IF NOT EXISTS)
  alter table if exists public.orders add column if not exists shipping_address jsonb;
  alter table if exists public.orders alter column customer_id drop not null;

  -- 1. Drop ALL existing policies before recreating (idempotent)
  -- Products
  ...
  -- 3. Migrate existing tables: add columns that may be missing from prior schema runs
  do $$ begin
    ...
    alter table public.products add column if not exists last_restocked timestamptz;
  ...
  -- 5. Updated-at trigger function + triggers
  ...
  -- Storage: public read, admin write (bucket must be created manually: name='product-images', public=true)
  ...
  -- 8. Helper: get admin by auth user id (bypasses RLS via SECURITY DEFINER)
  ...
  -- 9. Auto-sync registered users to public.customers
  ...
  -- 10. Backfill existing auth users into public.customers
  insert into public.customers (id, name, email, status)
  select
    id,
